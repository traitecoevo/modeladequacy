...
\singlespacing
\section{Abstract}
% Still think that the openning sentence could use more punch.  Will think on this.  Probably also on the long side generally (350 words).  Earlier in, try to get across that "a model that fits much better than an alternative can still be a poor fit to the data"
Phylogenetic comparative methods (PCMs) provide a historical perspective on evolutionary questions and are widely used test hypotheses with interspecific data. Inferences made using thes methods depend on a model describing the evolution of the trait along the phylogeny. The choice of the model is crucial --- the model needs to provide a good explanation for the data if inferences are to be reliable. Researchers routinely assessed the statistical power of a model by comparing the relative fit of a model to a small set of alternatives. We currently have no way to address a broader question: whether a model is a good absolute fit to the data.
We address two outstanding problems in comparative biology. First, we develop a general framework, based on Felsenstein's independent contrasts method, for assessing the absolute fit, or adequacy, of phylogenetic trait models. This approach can be applied to arbitrarily complex models of quantitative trait evolution. Second, we ask whether the simple models commonly used in PCMs are adequate descriptors of the macroevolutionary dynamics of real comparative data. We fit three common models of trait evolution to 337 comparative datasets covering 3 key Angiosperm functional traits (specific leaf area, seed mass and leaf nitrogen), compiled from the literature. Overall, we find these models to be very inadequate models for trait evolution at many different scales, especially so for large clades.This was true across many clades at many different scales though their adequacy was much worse at larger scales. Furthermore, the relative support for a model, as evaluated using conventional model selection criteria, had very little to do with absolute model adequacy. Our results raise very serious concerns about current practices in comparative biology. For many groups and traits, much more complex models than those commonly used will be necessary if we are to make robust macroevolutionary inference. Fortunately, the development of such models is well underway. Our approach can aid researchers in determining when and why further complexity needs to be invoked. We argue that assessing model adequacy should be a routine component of phylogenetic comparative studies.
...
% Wow; that's going to get some push back!  But it's punchy, I like it.
%
% Here, and probably elsewhere, avoid statements such as "unique and novel"; they really ask for descriptions of what is unique/novel (and it's not clear that uniqueness or novelty is better.  For example Pagel/Venditti's red queen exponential distribution paper is unique and novel and crap).  I've subbed in "important" here, but the statement would be good to solidify with a review of the comparative method.  Harvey's book would be fine here.  Or if you're looking for a non-PCM reference, not sure.
...
% MATT: This is the first part that needs actual work:

% The first part of this paragraph is really the scope of the models for which this works for; univariate continuous trait evolution models.
% This is lame, but please rewrite.
Phylogenetic comparative methods are used for exampling evolution of different sorts of traits, of speciation and extinction, of relationships between fossils and extant taxa \citep{Omeara2012} and a single framework for assessing adequacy for all such models seems out of reach at the present.  Here we consider one widely used case; that of the pattern of evolution of a single continuously valued trait.  To fit a model of this sort, we have a phylogenetic tree consisting of $n$ lineages. We have data on the the trait values observed at each tip $X$ ($X= X_1, X_2, \ldots, X_n$), but not generally at the nodes. In order to make inferences from the distribution of traits at the tips, we fit a model $\mathcal{M}$, describing the evolution of the trait along the phylogeny, and estimate a set of parameters $\Theta_{\mathcal{M}}$.
% This feels like an awkward way of introducing the Bayesian/frequentist divide.
Most analyses using comparative data aim to answer one of the following questions: what $\Theta_{\mathcal{M}}$  maximizes the probability of observing $X|\mathcal{M}$?; what is the posterior distribution of $\Theta_{\mathcal{M}}|X, \mathcal{M}$?; or, does $\mathcal{M}_1$ explain the data better than $\mathcal{M}_0$? 
% scope part ends here.
Our approach is conceptually distinct in that we want to ask, how likely is it that $\mathcal{M}$ would produce a dataset similar to $X$ if we re--ran evolution?

% I disagree with this aspect of the presentation; we've really got one approach; whether it is used in a frequentist or Bayesian way probably depends on whether the original analysis was frequentist/bayesian.  The way I see this (and this may be different to the way that you see this) is that people fit models as usual, but then use arbutus to assess the fit of the model.  So if one had fit the model using ML, then parametric bootstrapping is the way to estimate adequacy.  If one had fit the model with MCMC, then you're going to use a posterior predictive approach.  Not sure what the best way of getting that across is though.  The concern I have with the current presentation is that it confuses the bits that are ours with bits that are usual.
...
\end{equation}
where $\mathbb{P}(Y|X,\mathcal{M})$ is the probability of a new dataset $Y$ given $X$ and $\mathcal{M}$. We obtain this distribution by drawing repeated samples from $\mathbb{P}(\Theta_{\mathcal{M}}|X,\mathcal{M})$ and simulating datasets from the sampled parameters. We can compute the summary statistics $\mathcal{S}_X$ over the distribution of $\Theta_{\mathcal{M}}|X,\mathcal{M}$ and $\mathcal{S}_Y$ over $Y|\mathcal{M},X$ and compare $\mathcal{S}_X$ to $\mathcal{S}_Y$. Posterior predictive simulation approaches have also been developed for molecular phylogenetics \citep{Bollback2002, Reid2013, Lewis2013, Brown2013}, and recently for PCMs \citep{SlaterPennell}, but unfortunately, have not been widely adopted.

% About here is where I want to see a reference to the flow chart; then the summary statistics seem less oddly placed.  But something that gets the idea of fit a model, compute summary statstics, compare them part of the figure.  Some of the unclearness here though comes from the mixing above of fitting vs adequacy and of the splitting of frequentist/Bayesian.

\subsection{Summary statistics}
% It's not clear here why summary statistics are done using PICs; the iid part is made to seem like a nice extra rather than a requirement.  Can you rewrite this to start with what the problem first, and then note that working with the PICs solves it?
In order to compare the observed data $X$ with the data simulated under the fitted model $Y$ we need some way of summarizing the observations to make meaningful comparisons. 
% "data under the fitted model Y" makes it sound like Y is the fitted model.  "compare $X$, the observed data, with $Y$, the data simulated under the fitted model"
In our approach we calculate a set of summary statistics on a set of contrasts (\textit{sensu} Felsenstein \citep{Felsenstein1985}) computed at each node, rather than on the data itself. (We refer readers to \citep{Felsenstein1985, Rohlf2001, Blomberg2012}, for details on how contrasts are calculated.) Contrasts are a natural way of summarizing phylogenetically structured quantitative data with very little loss of information. Importantly, under Brownian motion (BM) the contrasts will be independent and identically distributed (i.i.d.) $\sim \mathcal{N}(0, \sigma)$, where $\sigma^2$ is the BM rate parameter \citep{Felsenstein1985}. The trait data at the tips can only be i.i.d. if there is no phylogenetic signal in the data. 

% I wonder if something about what makes for a good statistic would be good here so that these don't come from nowhere?
...
% Hmm, not sure about the motivation here, but at a loss to do better.
...
We compiled data on three traits from the literature: specific leaf area (SLA, defined as fresh area/dry mass); seed mass; and leaf nitrogen content (\% mass) (see Methods for details on datasets). SLA and leaf nitrogen content are important and widely measured components of the leaf economic spectrum, which relates to a plant's ecological strategy (i.e., how it balances photosynthetic capabilities and water cycling) \citep{Wright2004}. Seed mass is an important proxy for many life--history traits \citep{Westoby2002, Moles2005}. Understanding the macroevolutionary patterns of these three traits can provide key insights into the evolutionary processes that have shaped much of plant diversity \citep{ksi}.  

% Why 337?  Flip sentence around to say how the clades were chosen, but also *why*.  Remphasise that as trees get larger we expect that the models will become increasingly inadequate.  Possibly also have to head off concern about non-independence.  The time-slice thing is not clear; but clarify that you draw a line at some time back in time and take everything from there to the present?
We  matched these data to the phylogeny and then
%% Number of clades -- 337
pulled out 337 subclades, extracting named (families and orders) and unnamed clades taken from time--slices across the tree. Following Harmon et al. \citep{Harmon2010}, we fit three simple models of trait evolution to each subclade in the dataset: 1) BM, which can be associated with genetic drift \citep{Lande1976, Felsenstein1988, Lynch1990, HansenMartins1996}, randomly--varying selection \citep{Felsenstein1973}, or the summation of many independent processes over macroevolutionary time \citep{HansenMartins1996, Uyeda2011, PennellPE}; 2) single optimum OU, which is often assumed to represent stabilizing selection (following \citep{Lande1976}), though we think a more meaningful interpretation is that it represents an ``adaptive zone'' \citep{Hansen2012book, PennellHarmon}; and 3) EB, which was developed as a mathematical representation of a niche--filling process during an adaptive radiation
% should this citation list not include the original AC/DC paper?
\citep{Harmon2010, SlaterPennell}. We evaluated the relative support for each of the three models using conventional model selection techniques and then assessed the adequacy of the models using our procedure. (See Methods section for further details.) 


\subsection{A case study: seed mass evolution in the Meliaceae and Fagaceae }
% Are the radiations over similar amounts of time?
As an illustration of our approach, we present a case study examining seed mass evolution in two tree families, the Meliaceae (the mahogany family) and Fagaceae (which contains oaks, chestnuts and beech trees). The trait data and phylogeny for both groups are subsets of the larger dataset used in the analysis (see above, Methods). Superficially, these datasets are quite similar. Both are of similar size (Meliaceae: 44 species in dataset; Fagaceae: 70 species) and are ecologically comparable. As stated above, we fit three simple models of trait evolution (BM, OU, EB) to both datasets using maximum likelihood (ML) and computed AIC weights (AICw; \citep{Akaike1974, aicweight}) for the three models. For both datasets, an OU model was overwhelmingly supported (AICw $>$ 0.97 for both groups). Using the conventional model selection approach commonly used in comparative biology, we might conclude that similar evolutionary processes are important in these two clades of trees.

%Is the conventional model selection approach comparative comparative methods?
...
% Worth a comment about where, if anywhere, you'd expect a difference between ML and Bayesian approaches from the point of adequacy?  I feel like in cases where ML is picking stupid parameter estimates it might give quite a different picture.
...
% Does that mean that M_PIC is better because it detects violations more easily, or because it's more easily mislead?  I wonder if median(PIC) would be less sensitive that mean(PIC) -- could that be what is driving this?
...
% If you account for the size/age correlation does that persist?

\section{Discussion}

% I wonder if there is some parallel here about a linear regression that might get the point home.  If you fit a linear regression through a line that has curvature (e.g. y = x^2 on a positive domain) you can conclude that there is a significant positive slope, but the model is violated.  However it's really easy to spot that -- either just by looking at the data or diagnostic plots.  Heteroscedacity is a similar, but slightly harder to describe example.  The issue that we're trying to make the equivalent of the diagnostic plots...

The distinction between relative and absolute fit is an important one. A model may provide the the best explanation for a dataset compared to a few other models but still be a very poor explanation in terms of capturing the patterns of variation present in the data. 
...
% Whoa: "arbitrarily complex models of trait evolution".  Not even close until we can deal with discrete traits.   I'm not sure about either of the two sentences below here: actual results are more convincing.
The contribution of this our paper to this discussion is that the development of a general approach for empirically evaluating the adequacy of arbitrarily complex models of trait evolution. We demonstrate the concerns about model inadequacy are not just theoretical but have real implications for real data.

% At some point there was a nice triangle plot with the AIC values for BM, OU and EB that makes it a bit clearer what is going on.  Can we add that as SI?
...
% MWP: Good point, Will. I have tried to address this below. I think it now strikes a better balance.
%
% WKC: a bit too pessimistic for me here, for which I blame FitzJohn cynicism.  while the points about error are true, it is also possible there is something more 
% fundamental going on.  The other thing that OU does is provide a way for both rapid recent evolution (which we know occurs) 
% and to prevent the trait variance from increasing without bound forever.  I think the more forward looking way to do this is
% to focus on the parts of the pattern that OU describes better than BM and EB, and suggest that future (potentially) more adequate
% models will need to keep these good OU behaviors while improving elsewhere.  
%
% RGF: "FitzJohn cynicism".  The "trait variance from increasing without bound forever" is my bet for why OU tends to fit better than BM generally, but that's one side of the coin; that is only really a problem if there is evidence that trait evolution tends to be fairly rapid (high variance at the tips).  THe history erasing property of OU is also useful.  I like the paragraph above, but perhaps that's my cynicism.

% "This explanation" - spell it out so topic sentences are scannable.
%
% I tried to simplify the openning of the paragraph. Also, I think that in the era of epic SI, journals are phasing out "data not shown".  If we distribute the code, it will be there in the SI anyway.
This explanation is further supported by the how the observed summary statistics deviate from the simulated summary statistics.
%
...
% Is it worth describing the regression approach here or leave it till next paper?
...
% For the initial release, I suggest we hold off on CRAN and just link it off github using devtools to install.
...
% Here, and eleswhere, avoid "as stated elsewhere".  I've purged most of them, but a few remain.
