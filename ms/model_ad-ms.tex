...
% RGF: "an important one": show -- don't tell.  Why is it important one?
% RGF: "routinely assessed": not really
% RGF: Compress section starting "While not quantitative"
...
There are two primary ways of fitting models to comparative data. The first is use to obtain a point estimate of $\theta$ ($\hat \theta$), via maximum likelihood (ML), restricted maximum likelihood (REML), least--squares, etc. The second is to estimate posterior probability distribution $\Pr(\theta|X, \mathcal{M})$ using Bayesian approaches.
...
% RGF: This is the first mention of test statistics and I wonder if anything else  needs adding here.
While optimizing and Bayesian approaches to model--fitting are philosophically different from one another, our approach to assessing model adequacy is the same for both: 1) fit the model of trait evolution; 2) calculate a set of test statitics on the observed data $\mathcal{T}_X$; 3) simulate many new datasets $Y_1, Y_2, \ldots, Y_m$ under the model using the estimated parameters; 4) calculate test statistics on the simulated data $\mathcal{T}_{Y,1}, \mathcal{T}_{Y,2}, \ldots, \mathcal{T}_{Y,m}$; 5) compare $\mathcal{T}_X$ to the distribution of $\mathcal{T}_Y$. If $\mathcal{T}_X$ deviates significantly from the distribution of $\mathcal{T}_Y$, we can reject the model as inadequate (see figure \ref{fig:flowchart}).
...
% RGF: Other cases can be simplified as here:
...
% RGF: Why the prime here?  This is the first \nu in the paper
...
% RGF: make it clear here what the point is.  Transform a tree so that it works.  Make it clear the point that the reader can skip ahead if they want to trust you that the transformations work, and give the scope of models you'll cover.
...
% RGF: This is a terrible choice of notation!
...
% RGF: Seems we *could* average though?
...
Once we have created the unit tree from the estimated parameters, new datasets can be simulated under the model simply using a BM process with $\sigma^2 = 1$. 
% RGF: make the point that this is quick to do?
...
% RGF: My is more likely to be the abbreviation for million years.
...
Our method also applies to phylogenetic regression models \citep{Grafen1989, MartinsHansen1997}. Consider the linear model
\[Y = \beta_0 + \beta_1 X + \epsilon\]
where the error variance is structured by phylogeny assuming some model of trait evolution such that $\epsilon \sim \mathcal{N}(\mathbf{0}, \mathbf{V})$. Here $\mathbf{V}$ represents the variance--covariance matrix of the residuals rather than the traits \citep{Rohlf2001}. Therefore once a unit tree is constructed from $\mathbf{V}$, the contrasts on the residuals will be i.i.d. standard normal. This fact allows researchers to use our approach to assess the adequacy of a trait model for understanding correlations between traits. We note however that as $\mathbf{V}$ only affects the variance structure for these models, alternative apporaches \citep[see for example][ch. 6]{Gelmanbook} will be required to assess the adequacy of the mean structure of the model (i.e., $Y = \beta_0 + \beta_1$).
