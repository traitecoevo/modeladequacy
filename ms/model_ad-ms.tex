...
% Not the strongest start to the discussion.  Is there a stronger point to put here?
...
% There is error is true of every phylogenetic analysis ever--it's not specific to Zanne et al. or even "megaphylogenies"
%For out point in this paper, we need to say that phylogenetic uncertainty is strange in that it will tend to raise the estimates of the rates
% of evolution, but the effect on contrasts and the methods in arbutus are not obvious.  
%
% I would suggest building a paragraph on empirical sources of error and model adequacy maybe below in the Implications for empirical studies section
% I think it fits with the OU paragraph below.
% We just need to say we need more sophisticated ways of handling error, both in the traits and in the phylogeny, moving toward the future.  
% Moreover, we need to be able to separate error from model mis-specification
%
Likewise, the dataset we assembled is rather heterogeneous in terms of quality; the data was originally collected for a diverse set of reasons and some groups have been measured much more carefully than others. And while we have done our best to clean the data, errors undoubtedly remain. The test statistics outlined above can serve as useful diagnostics to aid researchers in identifying outliers that may be driving the pattern. Furthermore, we recommend that researchers faced with an inadequate model plot the magnitude of the contrasts on to the unit tree; this will usually be much more informative with regards to the model fit than plotting the magnitude of the contrasts on the original phylogeny. Exceptionally large or small contrasts on the unit tree can provide clues as to where the data may be erroneous.
...
% By "slop" you mean trait and phylogenetic error?  got to phrase this paragraph better.
% 
% here is some youtube biology for the day: http://youtu.be/vCyOVk5yY1s?t=26s 
%
% Extreme trait values don't do well, which I agree isn't the same as stabilizing selection, but the fact that OU doesn't allow the trait SD to increase
% to infinity does capture something real, although probably in a very crude way
% 
% I think to be more positive, would be to say something like:
% Future, and hopefully more adequate, models of trait evolution need to capture aspects of the OU model, especially the limit to the spread of trait values to a number less than infinity, while addressing some of its many issues, e.g. the time scale mis-match with stabilizing selection.  
%
