...
% RGF: I don't understand this as a motivation, or quite what is being argued here.  Surely nonidentifiability and things like that will plague both ML and MCMC analysis.  And if the motivation was that there might be important differences between MCMC and ML analyses they need highlighting clearly.
...
% RGF: This would have been my guess too.  There might be some issues with bias of parameters, but with uninformative or barely informative priors, that's not going to be surprising to most people.  I'd be inclined to reword the first paragraph -- there's no need to defend doing it both ways: they're both commonly used in comparative biology.
...
% RGF: Matt - you saw the figure that Owen Jones tweeted today: https://twitter.com/jonesor/status/445276320956944384.  The three panels aside from the top left indicate model inadequacy of different sorts, readily identifiable by plotting the data.  The problem is plotting phylogenetic data is hard and not ameniable to spotting departures from the model, so we need tools.  I think that this might be a reasonable way of motivating things: When you do a linear regression, you're meant to look at the residuals of the plot to make sure that your model is reasonable.
...
% RGF: I flipped this around to make the topic sentence first, but not totally happy with it.
...
% RGF: I found the previous version vague, but this is still not ideal; can you check carefully?  Basically trying to avoid use of "various" without examples, but don't want to put words into people's mouths.
...
% RGF: I'm sympathetic to this argument but it's hard to follow.
...
% RGF: the difference between this paragraph and the previous is not immediately apparent and the topic sentence feels waffly.  Is it possible to tighten the two paragraphs together as one?
...
% RGF: How much is the Quercus taxonomy likely to be hurting here?  It's a total mess, I think.
...
% RGF: Because this paragraph starts with "if" it invites a discussion of what happens if the reasons for departure are *not* biological.  What does that mean, and does that mean that modelling trait evolution is a lost cause?
...
% RGF: The advice here is a bit inconsistent, and I don't see how it's practical.  Is the argument that you can't look at the adequacy until you've made your model and then start completely over once you've found out that it's inadequate?  Isn't that as bad as looking at the adequacy as you go (you'll still just stop once you get an inadequate model).  Can you really not pick models that are likely to address the source of inadequacy (so if you think that the problem is rate variation select models like AUTEUR rather than (say) time dependent models).  I'm sure that there is a hard line philosophical view on this but I think that especially in light of how badly the models fit we have to come across as pragmatists.  I also feel that the "start over" approach here conflicts with the EDA approach in the case study.
...
% RGF: Tried to make this more explicit, not sure if it helps.
...
% RGF: Not sure what this means practically. Drop or expand?  And are you saying that these would be included *in addition* to the normal contrasts?  Won't that double count some departures so that the Mahalanobis distance inflates?  Or is that automatically accounted for by the covariance structure?
