...
% RGF: Disconnect here with the main title.
...
% RGF: I really like the new intro to the abstract.  Is it worth
% copying it here with: 
%
%     Box famously quipped: "Essentially, all models are wrong, but
%     some are useful".  Of course that means that some models can be
%     so badly wrong that they are useless.
%
% Of course that doesn't run well into the first paragraph of this
% version, so there'd need to be a bridge.  Something like
% "Surprisingly, despite their prevelance, there are few methods for
% assessing how well models fit..." or something.  Just wondering.
...
% RGF: I've normalised a few notational issues here, but others remain:
% * l.c. \theta instead of \Theta would be more typical.
% * drop dependence on M for theta, because it never appears without
%   it (it's only useful if the dependency varies).
...
There are two primary ways of fitting models to comparative data. The first is use maximum likelihood (ML), restricted maximum likelihood (REML), least--squares, etc. to obtain a point estimate of $\Theta_{\mathcal{M}}$. The second is to estimate posterior probability distribution $\Pr(\Theta_{\mathcal{M}}|X, \mathcal{M})$ using Bayesian appraoaches.
For the models used in comparative biology, estimating $\Pr(\Theta_{\mathcal{M}}|X, \mathcal{M})$ requires using Markov chain Monte Carlo (MCMC) machinery to sample values of $\Theta_{\mathcal{M}}$.   

%% Overview of approach
% RGF: There is still an issue here in that "our approach can be
% applied" comes before the explanation of what the approach is.  This
% is a bit odd, but perhaps it's just me.
...
%% Bayesian inference
If we have a posterior probability distribution $\Pr(\Theta_{\mathcal{M}}|X, \mathcal{M})$, we can assess model adequacy using posterior predictive simulation \citep{Rubin1984, Gelman1996}. We obtain new datasets by sampling from a second distribution, the posterior predictive distribution
% RGF: Just double check this is '=' not '\propto'.
\begin{equation}
\Pr(Y|X,\mathcal{M}) = \int \Pr(Y|\Theta_{\mathcal{M}}, \mathcal{M})\Pr(\Theta_{\mathcal{M}}|X,\mathcal{M})d\Theta_{\mathcal{M}}
\end{equation}
where $\Pr(Y|X,\mathcal{M})$ is the probability of a new dataset $Y$ given $X$ and $\mathcal{M}$, averaged over hthe distribution of the parameters. In practice, sampling from $\Pr(Y|X,\mathcal{M})$ entails drawing samples from the joint posterior distribution of parameters $\Pr(\Theta_{\mathcal{M}}|X, \mathcal{M})$ and simulating data on the phylogeny according to the sampled parameter values. Therefore, the datasets $Y_1, Y_2, \ldots, Y_m$ are each generated from different values of $\Theta_{\mathcal{M}}$.  
% There is a difference here with the above paragraph.  Finish describing how it works before going into the connections with previous work.
...
% I wonder if going straight for the unit tree would be better here so
% we immediately have N(0,1).  That's just rescaling contrasts by
% \sigma^2?
